...
def building_model(model_arch, hidden_units, dropout):
    ...
    if isinstance(last_layer, nn.Sequential):
        first_linear = [module for module in last_layer.modules() if isinstance(module, nn.Linear)][0]
...
def training_classifier(model, trainloader, validloader, testloader, learning_rate, epochs, use_gpu):
    ''' 
    ...
    for e in range(epochs):
        ...
        for inputs, labels in trainloader:
            ...
            if no_batches % print_every == 0 or no_batches == len(trainloader):
                ...
                v_tot_time = v_end_time - v_start_time #calculate difference between end time and start time
                print("Validating time:",
                      str(int((v_tot_time/3600)))+":"+str(int((v_tot_time%3600)/60))+":"
        ...
        t_tot_time = t_end_time - t_start_time #calculate difference between end time and start time
        print("Epoch time:",
              str(int((t_tot_time/3600)))+":"+str(int((t_tot_time%3600)/60))+":"
              +str(int((t_tot_time%3600)%60))+"\n")
    
    return model, optimizer.state_dict()

def saving_model(save_dir, class_to_idx, trained_model, optimizer_state_dict, arch,
                 learning_rate, hidden_units, dropout, epochs):
    ''' 
