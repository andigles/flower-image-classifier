...
def get_train_args():
    ...
    parser.add_argument('--arch', type = str, default = 'resnet50', help = 'The pretrained model architecture to use')
    parser.add_argument('--learning_rate', type = float, default = 0.0005, help = 'Learning rate value used for training')
    parser.add_argument('--hidden_units', nargs='*', type = int, default = 512, help = 'Hidden units e.g. --hidden_units 512 256 64')
    parser.add_argument('--dropout', type = float, default = 0.2, help = 'Dropout value for overfitting')
    parser.add_argument('--epochs', type = int, default = 5, help = 'Number of epochs used for training')
    parser.add_argument('--gpu', action='store_true', help = 'If activated, run training in GPU mode if available')
...
def get_predict_args():
    ...
    parser.add_argument('--top_k', type = int, default = 1, help = 'Int number for k most likely classes') 
    parser.add_argument('--category_names', type = str, help ='Mapping to categorical name in path to file')
    parser.add_argument('--gpu', action='store_true', help = 'If activated, run training in GPU mode if available')
